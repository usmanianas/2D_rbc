...
def TimeIntegrate():
    ...
    while True:
        ...
        if abs(time - tMax)<1e-5:
            writeSoln(U, W, P, T, time)
            Z, X = np.meshgrid(x,z)
            plt.contourf(X, Z, T[int(Nx/2), :], 500, cmap=cm.coolwarm)
            clb = plt.colorbar()
            plt.quiver(X[0:Nx,0:Nz], Z[0:Nx,0:Nz], U[int(Nx/2), :], W[int(Nx/2), :])
            plt.axis('scaled')
...
def computeNLinDiff_X(U, W):
    ...
    Hx[1:Nx-1, 1:Nz-1] = (((U[2:Nx, 1:Nz-1] - 2.0*U[1:Nx-1, 1:Nz-1] + U[0:Nx-2, 1:Nz-1])/hx2 + 
                              ...
                              U[1:L-1, 1:N-1]*(U[2:Nx, 1:Nz-1] - U[0:Nx-2, 1:Nz-1])/(2.0*hx) -
                              W[1:L-1, 1:N-1]*(U[1:Nx-1, 2:Nz] - U[1:Nx-1, 0:Nz-2])/(2.0*hz))
...
def uJacobi(rho):
    ...
    while True:
        ...
        Up[1:Nx-1, 1:Nz-1] =(1.0/(nu*dt*(idx2 + idz2))) * (rho[1:Nx-1, 1:Nz-1] + 
                                       0.5*nu*dt*idx2*(Up[0:Nx-2, 1:Nz-1] + Up[2:Nx, 1:Nz-1]) +
                                       0.5*nu*dt*idz2*(Up[1:Nx-1, 0:Nz-2] + Up[1:Nz-1, 2:Nz]))   


        imposeUBCs(Up)
        
        maxErr = np.amax(np.fabs(rho[1:Nx-1, 1:Nz-1] - (Up[1:Nx-1, 1:Nz-1] - 0.5*nu*dt*(
                            (Up[0:Nx-2, 1:Nz-1] - 2.0*Up[1:Nx-1, 1:Nz-1] + Up[2:Nx, 1:Nz-1])/hx2 +
                            (Up[1:Nx-1, 0:Nz-2] - 2.0*Up[1:Nx-1, 1:Nz-1] + Up[1:Nx-1, 2:Nz])/hz2))))
...
def wJacobi(rho):
    ...
    while True:
        ...
        Wp[1:Nx-1, 1:Nz-1] =(1.0/(nu*dt*(idx2 + idz2))) * (rho[1:Nx-1, 1:Nz-1] + 
                                       0.5*nu*dt*idx2*(Wp[0:Nx-2, 1:Nz-1] + Wp[2:Nx, 1:Nz-1]) +
                                       0.5*nu*dt*idz2*(Wp[1:Nx-1, 0:Nz-2] + Wp[1:Nz-1, 2:Nz]))   
    
        imposeWBCs(Wp)


        maxErr = np.amax(np.fabs(rho[1:Nx-1, 1:Nz-1] - (Wp[1:Nx-1, 1:Nz-1] - 0.5*nu*dt*(
                        (Wp[0:Nx-2, 1:Nz-1] - 2.0*Wp[1:Nx-1, 1:Nz-1] + Wp[2:Nx, 1:Nz-1])/hx2 +
                        (Wp[1:Nx-1, 0:Nz-2] - 2.0*Wp[1:Nx-1, 1:Nz-1] + Wp[1:Nx-1, 2:Nz])/hz2))))
...
def TJacobi(rho):
    ...
    while True:
        ...
        T[1:Nx-1, 1:Nz-1] =(1.0/(kappa*dt*(idx2 + idz2))) * (rho[1:Nx-1, 1:Nz-1] + 
